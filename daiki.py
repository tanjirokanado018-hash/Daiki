...
import ddddocr  # <-- NEW: CAPTCHA solver
...
DEBUG = True  # <-- NEW: enable debug logs to admin
...
async def solve_captcha(session, base_url, sid, debug=False):
    """
    Download CAPTCHA image and solve using ddddocr.

    Args:
        session: aiohttp.ClientSession
        base_url: e.g., "https://portal-as.ruijienetworks.com"
        sid: sessionId
...
# ---------------------- WORKER TASK (with CAPTCHA support) ----------------------
async def worker_task(user_id, app_session):
    ...
    while not session.stop_event.is_set():
        ...
        try:
            ...
            async with app_session.post(api_url, json=payload, headers=headers, timeout=5, ssl=False) as r:
                ...
                # ----- CASE 3: RATE LIMITED -----
                    ...
                    my_sid = None  # Force session refresh
                    continue
        ...
        # Delay with jitter to avoid rate limiting
...
def get_user_keyboard(user_id):
    ...
    markup.add(
        KeyboardButton(" ADD PORTAL URL"),
        KeyboardButton(" MY URLS")
    )
    markup.add(
        KeyboardButton("⚙️ SETUP CONFIG MOD"),
        KeyboardButton(" START SCAN")
    )
...
async def cmd_start(message):
    ...
    if not data_manager.is_authorized(user_id):
        await bot.reply_to(
            ...
            "⚠️  ကျေးဇူးပြု၍ အောက်ပါ Menu မှတစ်ဆင့် ဝယ်ယူပါ ",
...
async def handle_bot_inputs(message):
    ...
    if message.content_type == 'text':
        ...
        if text_data == " Ngwe Lwe Pyay Sar Po Yan":
            await bot.reply_to(message, " ကျေးဇူးပြု၍ Ngwe Lwe Pyay Sar (Screenshot Photo) အား တိုက်ရိုက် ပို့ပေးပါခင်ဗျာ။")
            return
        ...
        if not data_manager.is_authorized(user_id):
            await bot.reply_to(
                message,
                "❌ သင့်တွင် ဤ Bot အား သုံးခွင့်မရှိသေးပါ။ ပြေစာ ပေးပို့ပေးပါ။",
                reply_markup=get_unauth_keyboard()
            )
            return
...
async def main():
    print("[*] Starting Premium Daiki Code Hack Bot Platform...")
